...
void MyRun::RecordEvent(const G4Event* evt)
{
  ...
  if (HC1) {
    ...
    for (int i = 0; i < n_hit; i++) {
      ...
      G4double vertexenergy = (*HC1)[i]->GetVertexEnergy();

      G4double Edep = (*HC1)[i]->GetEdep();

      G4ThreeVector vertexpos = (*HC1)[i]->GetVertexPosition();
      ...
      hitFile << "\n"
              ...
              << "," << vertexenergy / keV << "," << Edep / keV << "," << vertexpos.x() / cm << "," << vertexpos.y() / cm  << "," << vertexpos.z() / cm;
      hitFile.close();
    }
  }
