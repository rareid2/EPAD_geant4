...
PrimaryGeneratorMessenger::PrimaryGeneratorMessenger(
  ...
  fPrimDir->SetGuidance("Select trapped background, loss cone background, or "
                        "photon signal source.");

  // Sets fWhichParticle
  fcmd = new G4UIcmdWithAnInteger("/particleSource/setBackgroundType", this);
  fcmd->SetParameterName(
      "Background Type {0,1,2,3} (Trapped, Backscattered Loss Cone, Signal "
      "Photons, Signal photons-other distributions)",
      true);
  ...
  // Sets fDistType
  fcmd2 =
      new G4UIcmdWithAnInteger("/particleSource/setSpatialDistribution", this);
  ...
  fcmd3 = new G4UIcmdWithAnInteger(
      "/particleSource/setBackgroundSpatialDistribution", this);
  fcmd3->SetParameterName("Trapped electron spatial distribution {0,1,2} "
                          "(sine, sine 2 theta, isotropic).",
                          true);
